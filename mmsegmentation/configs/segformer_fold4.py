...
train_dataloader = dict(
    ...
    dataset=dict(
        ...
        data_root='data/Satellite',
        data_prefix=dict(
            img_path='img_dir/real_train_img',
            seg_map_path='ann_dir/train_gt'),
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(type='LoadAnnotations', reduce_zero_label=False),
            dict(type='RandomCrop', crop_size=(
...
val_dataloader = dict(
    ...
    dataset=dict(
        ...
        data_root='data/Satellite',
        data_prefix=dict(
            img_path='img_dir/fold4_val_slice_img',
            seg_map_path='ann_dir/fold4_val_slice_gt'),
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(type='Resize', scale=(
                512,
...
test_evaluator = dict(
    ...
    format_only=True,
    output_dir='_result_img/segformer_class2')
default_scope = 'mmseg'
env_cfg = dict(
...
train_cfg = dict(
    type='IterBasedTrainLoop', max_iters=100000, val_interval=3000)
...
launcher = 'none'
work_dir = '_satellite_new/segformer_lossadd'
