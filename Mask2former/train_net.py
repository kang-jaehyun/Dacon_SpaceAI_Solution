...
from mask2former.evaluation.sem_seg_evaluation import SemSegEvaluator # add dice score
...
class Trainer(DefaultTrainer):
    ...
    def build_train_loader(cls, cfg):
        ...
        # coco panoptic segmentation lsj new baseline
            ...
            return build_detection_train_loader(cfg, mapper=mapper)
        elif cfg.INPUT.DATASET_MAPPER_NAME == "dacon_instance":
            mapper = DACONInstanceDatasetMapper(cfg, True)
            return build_detection_train_loader(cfg, mapper=mapper)
        else:
            mapper = None
            return build_detection_train_loader(cfg, mapper=mapper)
    ...
    @classmethod
    def inference_with_TTA(cls, cfg, model):
        def rle_encode(mask):
            pixels = mask.flatten()
        ...
        for batched_inputs in data_loader:
            ...
            for mask_cls_result, mask_pred_result, input_per_image, image_size in zip(
                ...
                if model.semantic_on:
                    r = retry_if_cuda_oom(model.semantic_inference)(mask_cls_result, mask_pred_result)
                    if not model.sem_seg_postprocess_before_inference:
                        r = retry_if_cuda_oom(sem_seg_postprocess)(r, image_size, height, width)
                    processed_results[-1]["sem_seg"] = r
            for result in processed_results:
                mask = result['sem_seg'].argmax(dim=0).byte().cpu().numpy()
                rle = rle_encode(mask)
                if rle == '':
                    rle = -1
                lst.append(rle)
...
def main(args):
    ...
    if args.eval_only:
        ...
        # return res
        model = Trainer.build_model(cfg)
        Trainer.inference_with_TTA(cfg,model)
        return None
    trainer = Trainer(cfg)
